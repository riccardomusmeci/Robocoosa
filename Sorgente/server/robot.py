...
class Robot(object):
    ...
    def __init__(self):
        ...
        self.address = "rtsp://192.168.1.4:554/live/ch00_0"
        ...
        self.oggetto_trovato = 0
        self.presaRilascio = 0
        self.area_trovata = 0
        self.oggettoPreso = 0
        self.areaRaggiunta = 0
    ...
    def analyzeDataFromCamera(self):
        ...
        '''
        print "trovaOggetto: ", self.oggetto_trovato
        print "trovaArea: ", self.area_trovata
    
        if self.oggetto_trovato == 1 and self.modalita == "movimento" and self.oggettoPreso == 0:
            print "Sono in modalita avvicinamento"
            self.modalita = "avvicinamento"
            return

        if self.area_trovata == 1 and self.modalita == "movimento" and self.oggettoPreso == 1:
            print "Sono in modalita avvicinamento per l'area"
            self.modalita = "avvicinamento"
            return

        if self.presaRilascio == 1 and self.oggettoPreso == 0 and self.modalita == "avvicinamento":
            self.modalita = "presa-rilascio"
            self.presaRilascio = 0
            self.oggettoPreso = 1
            return

        if self.presaRilascio == 1 and self.areaRaggiunta == 0 and self.modalita == "avvicinamento":
            self.modalita = "presa-rilascio"
            self.presaRilascio = 0
            self.areaRaggiunta = 1
            return
    ...
    def trovaColore(self, colore):
        print "trovaColore"
        mask = cv2.inRange(self.hsv, colore.lower, colore.upper)
    ...
    def trovaOggetto(self, targetColor):
        print "trovaOggetto"
        ...
        if len(cnts) > 0 and self.trovaColore(self.oggettoBlu):
            c = max(cnts, key=cv2.contourArea)
            ((x, y), radius) = cv2.minEnclosingCircle(c)
            M = cv2.moments(c)
            center = (int(M["m10"] / M["m00"]), int(M["m01"] / M["m00"]))
            ...
            if radius > 12:
                self.oggetto_trovato = 1


    def trovaArea(self, targetColor):
        print "trovaArea"
        mask = cv2.inRange(self.hsv, targetColor.lower, targetColor.upper)
        mask = cv2.erode(mask, None, iterations=2)
        mask = cv2.dilate(mask, None, iterations=2)
    ...
    def elaboraFrame(self, frame):
        ...
        if self.oggetto_trovato == 0 and self.modalita == 'movimento':
            self.trovaOggetto(self.oggettoRosso)
        
        if self.oggetto_trovato == 1 and self.presaRilascio == 0 and self.modalita == 'avvicinamento':
            self.versoOggetto(self.oggettoRosso)

        if self.oggetto_trovato == 1 and self.oggettoPreso == 1 and self.modalita == 'movimento':
            self.trovaArea(self.oggettoRosso)

        if self.area_trovata == 1 and self.presaRilascio == 0 and self.modalita == 'avvicinamento':
            self.versoOggetto(self.oggettoRosso)

        if self.area_trovata == 1 and self.presaRilascio == 1:
